##############################################################################
package App::Page::Worklist::Document::Labs;
##############################################################################

use strict;
use SDE::CVS ('$Id: Labs.pm,v 1.1 2000-11-08 02:44:34 robert_jenks Exp $', '$Name:  $');
use CGI::Dialog::DataNavigator;
use SQL::GenerateQuery;
use App::Configuration;
use Data::Publish;

use App::Page::Worklist::Document;
use base qw(App::Page::Worklist::Document);
...
%RESOURCE_MAP = (
	'worklist/documents/labs' => {
		_idSynonym => '_default',
		_title => 'Lab Results Work List',
...
sub obsQuery
{
	...
	my $cond2 = $sqlGen->WHERE('observer_id', 'is', $self->session('person_id'));
