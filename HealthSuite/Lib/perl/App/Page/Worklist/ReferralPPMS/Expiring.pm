##############################################################################
package App::Page::Worklist::ReferralPPMS::Expiring;
##############################################################################
...
%RESOURCE_MAP = (
	'worklist/referralppms/expiring' => {
		_idSynonym => ['_expiring'],
		_title => 'Expiration Referrals',
		...
		_views => [
				{caption => 'Work List' , name => 'expiring',},
...
%PUB_REFERRAL_EXP = (
	...
	dnAncestorFmt => 'Expiration Referrals Worklist',
);
...
# Expiry view
########################################################
sub prepare_view_expiring
{
	my $self = shift;

	my $tabsHtml = $self->setupTabs();

	my $dlg = new CGI::Dialog::DataNavigator(publDefn => \%PUB_REFERRAL_EXP, topHtml => $tabsHtml, page => $self);
