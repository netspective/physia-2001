...
sub initialize
{
	...
	my $rootFS = File::Spec->catfile($CONFDATA_SERVER->path_OrgReports(), 'General');
	...
	my @activePathInfo = App::Component::Navigate::FileSys::getActivePathInfo(0, $rootFS, '/report', \@activePath, 'locator');
		...
		<TABLE WIDTH=100% BGCOLOR=LIGHTSTEELBLUE CELLSPACING=0 CELLPADDING=3 BORDER=0>
			...
			<TD ALIGN=RIGHT>
				<FONT FACE="Arial,Helvetica" SIZE=2>
					<A HREF="#hrefSelfNoDlg#">Edit</A> | <A HREF=".">Reports List</A>
				</FONT>
			</TD>
			</TR>
			} : ''
...
sub handleDrillDown
{
	...
	# if the report doesn't know how to handle drill downs, fail
		...
		unless($dlgFlags & CGI::Dialog::DLGFLAG_EXECUTE)
		{
			$self->addContent(qq{
				<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0 WIDTH=100%>
					...
					<TD>@{[ $html ]}</TD>
					<TD WIDTH=10>&nbsp;</TD>
					<TD ALIGN=RIGHT>#component.navigate-reports#</TD>
					</TR>
				</TABLE>
				});
		}
	...
	else
	{
		$self->addContent(qq{
			<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0>
				<TR VALIGN=TOP>
				<TD>#component.navigate-reports#</TD>
				</TR>
			</TABLE>
			});
	}
