...
sub getForm
{
	...
	if($self->param('execute') && ! ($flags & (SEARCHFLAG_LOOKUPWINDOW | SEARCHFLAG_SEARCHBAR)))
	{
		$itemFns = qq{
			...
			<SELECT name="item_action_arl_select">
				...
				<option value="/person/%itemValue%/dlg-prescribe-medication">Prescribe Medication</option>
	...
	$searchType = 'insured-person' if $searchType eq 'insured-Person';
