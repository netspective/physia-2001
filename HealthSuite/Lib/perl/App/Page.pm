...
use enum qw(BITMASK:PAGEFLAG_ ISDISABLED ISPOPUP ISADVANCED ISFRAMESET ISFRAMEHEAD ISFRAMEBODY ISHANDHELD IGNORE_BODYHEAD IGNORE_BODYFOOT CONTENTINPANES INCLUDEDEFAULTSCRIPTS RECORD_VIEWCOUNT CUSTOM);
...
sub printContents
{
	...
	# We don't print anything if a custom page has already been displayed
	return if $self->flagIsSet(PAGEFLAG_CUSTOM);
