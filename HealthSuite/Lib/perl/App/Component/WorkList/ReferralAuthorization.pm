##############################################################################
package App::Component::WorkList::ReferralAuthorization;
##############################################################################
...
%RESOURCE_MAP = (
	'worklist-referral-authorization' => {
		_class => new App::Component::WorkList::ReferralAuthorization(),
		},
	);
...
sub getComponentHtml
{
	...
	my $referralAuth;
	$referralAuth = $STMTMGR_COMPONENT_REFERRAL->getRowsAsHashList($page, STMTMGRFLAG_NONE, 'sel_referral_authorization', $page->session('org_internal_id'));
	...
	$html .= "<i style='color=red'>No Referral data found.</i> <P>"
