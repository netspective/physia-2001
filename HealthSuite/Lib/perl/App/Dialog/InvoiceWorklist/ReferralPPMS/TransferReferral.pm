##############################################################################
package App::Dialog::InvoiceWorklist::ReferralPPMS::TransferReferral;
##############################################################################
...
%RESOURCE_MAP = ('transfer-referral' => {heading => 'Transfer Referral',  _arl => ['person_id','referral_id'],},);
...
sub new
{
	...
	$self->addContent(
			...
			new App::Dialog::Field::Person::ID(incSimpleName=>1, name => 'transfer_id', caption =>'Transfer To', options => FLDFLAG_REQUIRED, hints => 'Person to Transfer Referral'),
		...
		$self->{activityLog} =
		{
			...
			scope =>'transaction',
			key => "#param.person_id#",
			data => "Transfer Referral  to <a href='/person/#param.person_id#/profile'>#param.person_id#</a>"
		};
