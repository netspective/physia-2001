##############################################################################
package App::Dialog::Directory::Org::General::VendorNameLookup;
##############################################################################
...
sub new
{
	my $self = App::Dialog::Directory::new(@_, id => 'provider-vendor-name', heading => 'Vendor Name');

	$self->addContent(
			new CGI::Dialog::Field(caption => 'Vendor Name',  name => 'vendorname'),
			);
