##############################################################################
package App::Dialog::InsuredPerson;
##############################################################################
...
%RESOURCE_MAP = (
	'insured-Person' => {
		heading => '$Command Insured Person',
		_arl => ['party_name'],
		_arl_modify => ['party_name'],
		_idSynonym => 'Insured-Person'
	},
);

sub new
{
	my ($self, $command) = CGI::Dialog::new(@_, id => 'insured-Person', heading => 'Insured Person');
	...
	$self->addContent(
		...
		new App::Dialog::Field::Person::ID::New(caption => 'Person/Patient ID', name => 'person_id', readOnlyWhen => CGI::Dialog::DLGFLAG_UPDORREMOVE, types => ['Insured-person'],),
...
sub populateData
{
	...
	my $relationName = 'Insured-person';
