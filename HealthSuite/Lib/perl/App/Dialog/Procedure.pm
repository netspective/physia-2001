...
sub hmoCapWriteoff
{
	...
	foreach my $proc (@{$procItems})
	{
		...
		$page->schemaAction(
			...
			value_text => $batchInfo->{value_text} || undef,
			value_date => $todaysDate || undef,
...
sub storeFacilityInfo
{
	...
	my $billingFacilityAddr = $STMTMGR_ORG->getRowAsHash($page, STMTMGRFLAG_CACHE, 'selOrgAddressByAddrName', $billFacilityId, 'Payment');
