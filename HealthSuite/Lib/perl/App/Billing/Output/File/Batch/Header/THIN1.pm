...
sub formatData
{
	...
	my $billingFacility = $firstClaim->{payToOrganization};
	my $billingProvider = $firstClaim->{payToProvider};

	my $emcId;
	my $taxId;
	my $taxIdType;

	if ($billingFacility->getTaxId() ne '')
	{
		$taxId = $billingFacility->getTaxId();
		$taxId =~ s/-//g;
		$taxIdType = 'E';
	}
	else
...
my %payerType = ( THIN_MEDICARE . "" =>
	  ...
	  substr($billingFacility->getMedicareId(),0,15), # provider medicare no.
	  ...
	  $spaces, # substr($claimRenderingProvider->getPIN(),0,6), # prov UPIN-USIN id
	  $spaces, # reserved filler
    substr($billingFacility->getMedicaidId(),0,15), # medicaid no.
	  $spaces, # prov champus no
	  substr($billingFacility->getBCBSId(),0,15), # blueshield no
	  $spaces, # prov commercial no.
	  $spaces, # prov no 1
	  $spaces, # prov no 2
	  substr(($billingFacility->getTaxTypeId() =~ /['E','X']/) ? $firstClaim->{billingFacility}->getName() : $spaces ,0,33),
	  substr(($billingFacility->getTaxTypeId() eq 'S') ? $billingProvider->getLastName() : $spaces,0,20),
	  substr(($billingFacility->getTaxTypeId() eq 'S') ? $billingProvider->getFirstName(): $spaces,0,12),
	  substr(($billingFacility->getTaxTypeId() eq 'S') ? $billingProvider->getMiddleInitial(): $spaces,0,1),
	  substr($billingProvider->getSpecialityId(),0,3), # speciality code
