...
my %payerType = ( THIN_MEDICARE . "" =>		
	  ...
	  substr(($claimPayToProvider->getTaxTypeId() eq 'S') ? $claimRenderingProvider->getLastName() : $spaces,0,20),
	  substr(($claimPayToProvider->getTaxTypeId() eq 'S') ? $claimRenderingProvider->getFirstName(): $spaces,0,12),
	  substr(($claimPayToProvider->getTaxTypeId() eq 'S') ? $claimRenderingProvider->getMiddleInitial(): $spaces,0,1),
