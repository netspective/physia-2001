...
sub processClaim
{
	...
	my $recordCreationCondition =
	{
		NSF_ENVOY . "" =>
		{
			...
			'DA2' => sub {
				if(
					($tempClaim->{insured}->[$tempClaim->getClaimType()]->{address}->getAddress1() ne $tempClaim->{careReceiver}->{address}->getAddress1()) &&
			...
			'DAat' => sub {
		    if (($tempClaim->getFilingIndicator() =~ /['M','P']/ ) || ($tempClaim->getSourceOfPayment() =~ /['G','P']/))
				{ return 1; }
				else { return 0;}
			},
			'EA0' => sub { return 1;},
