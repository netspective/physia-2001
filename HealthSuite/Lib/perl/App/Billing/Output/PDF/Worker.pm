...
sub box31ClaimData
{
	...
	die "Couldn't set font"  if ($font == -1);
	pdflib::PDF_setfont($p, $font, DATA_FONT_SIZE);
	pdflib::PDF_show_xy($p , $billingProvider->getName(), $box31X + CELL_PADDING_X + 0, START_Y + 20);
	pdflib::PDF_show_xy($p , $billingProvider->getProfessionalLicenseNo(), $box31X + CELL_PADDING_X + 0, START_Y + 12);
