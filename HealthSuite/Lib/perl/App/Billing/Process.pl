...
# For Oracle 8
#**************************************************************************
use constant cntStr => 'dbi:Oracle:SDEDBS03';
				...
				valMgr => $valMgr))
	{
		if($valMgr->haveErrors() == 0)
		{
			...
			if($valMgr->haveErrors() == 0)
			{
				...
				{
		  			 # PRINT TEST.TXT FILE
		  			 my $outResult = $output->processClaims(destination => NSFDEST_FILE, outArray => \@outArray, outFile => './test.txt',claimList => $claimList, validationMgr => $valMgr, nsfType => NSF_HALLEY);
