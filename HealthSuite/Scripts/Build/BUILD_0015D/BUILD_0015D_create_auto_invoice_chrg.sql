...
(nvl(
	     (
              SELECT max(ia.value_date) FROM invoice_attribute ia WHERE ia.item_name = 'Invoice/Payment/Batch ID' 
	...
	(nvl(
	     (
              SELECT max(ia.value_text) FROM invoice_attribute ia WHERE ia.item_name = 'Invoice/Payment/Batch ID' 
	     AND ia.value_int = iia.adjustment_id
             )
            ,'AUTO_CREATE_ID'
	     )),			
