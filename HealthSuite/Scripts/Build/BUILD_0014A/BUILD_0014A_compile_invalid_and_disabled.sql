...
select object_type, object_name, status from user_objects where status='INVALID' order by object_type;

select trigger_name, status from user_triggers where status='DISABLED' order by trigger_name;

select constraint_name, constraint_type, status from user_constraints where status='DISABLED'
