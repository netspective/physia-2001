package dialog.field.insurance.plan;
...
public class NewField extends DialogField {
	...
	public boolean isValid (DialogContext dc) {
		...
		String planName = dc.getValue(idTextField);
        ...
        if (!planName.equalsIgnoreCase("")) {
			try {
				status = stmtMgr.stmtRecordExists(dbContext, dc, dataSrcId, "insurance.selNewPlanExists", new Object[] { planName, ownerOrgId });
			} catch (Exception e) {
				status = false;
		...
		if (!status) {
			invalidate(dc, "Plan Name " + planName + " already exists.");
		}
